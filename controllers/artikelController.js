    ...
    static async putArtikel(req, res) {
        try {
            ...
            if (artikel) {
                ...
                res.status(200).json({ msg: `Data Artikel dengan id ${id} Berhasil di Update!` });
            } else {
                res.status(404).json({ msg: `Artikel Dengan ID ${id} Tersebut Tidak Di Temukan!` });
            }
        } catch (error) {
            res.status(500).json({ msg: 'Terjadi Kesalahan Saat Mengupdate Data Artikel!' });
        }
    }
}
